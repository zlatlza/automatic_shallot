...
# Function to handle adding a chord with settings
def add_chord(chord_name_var, chord_type_var, octave_var, arpeggio_var, apply_settings_var, chords, chord_listbox, duration_entry, bpm_entry, time_signature_entry, volume_entry, reverb_entry, echo_delay_entry, echo_decay_entry, chords_var, drums_var):
    ...
    if chord_name in CHORD_FREQUENCIES and chord_type in CHORD_TYPES:
        chords.append((chord_name, chord_type, arpeggio, include_drums, include_chords, duration, bpm, time_signature, volume, reverb_decay, echo_delay, echo_decay, octave))
...
def update_chord(chord_name_var, chord_type_var, octave_var, arpeggio_var, apply_settings_var, chords, chord_listbox, duration_entry, bpm_entry, time_signature_entry, volume_entry, reverb_entry, echo_delay_entry, echo_decay_entry, chords_var, drums_var):
...
def save_chord_settings():
    update_chord(chord_name_var, chord_type_var, octave_var, arpeggio_var, apply_settings_var, chords, chord_listbox, duration_entry, bpm_entry, time_signature_entry, volume_entry, reverb_entry, echo_delay_entry, echo_decay_entry, chords_var, drums_var)
...
generate_button.grid(column=0, row=13, columnspan=7, sticky=(tk.W, tk.E))
...
save_button.grid(column=0, row=14, columnspan=3, sticky=(tk.W, tk.E))
...
load_button.grid(column=3, row=14, columnspan=3, sticky=(tk.W, tk.E))

preview_chord_button = ttk.Button(frame, text="Preview Chord", command=lambda: preview_chord(chords, chord_listbox))
preview_chord_button.grid(column=6, row=7, sticky=(tk.W, tk.E))

preview_song_button = ttk.Button(frame, text="Preview Song", command=lambda: preview_song(chords, chords_var, drums_var, noise_var))
preview_song_button.grid(column=6, row=8, sticky=(tk.W, tk.E))

status_label = ttk.Label(frame, text="")
status_label.grid(column=0, row=15, columnspan=7, sticky=(tk.W, tk.E))
...
copy_button.grid(column=6, row=4, sticky=(tk.W, tk.E))

paste_button = ttk.Button(frame, text="Paste", command=paste_chord)
paste_button.grid(column=6, row=5, sticky=(tk.W, tk.E))

move_up_button = ttk.Button(frame, text="Move Up", command=move_up)
move_up_button.grid(column=6, row=6, sticky=(tk.W, tk.E))

move_down_button = ttk.Button(frame, text="Move Down", command=move_down)
move_down_button.grid(column=6, row=7, sticky=(tk.W, tk.E))
